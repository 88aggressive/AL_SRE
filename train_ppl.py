...
model,loss_func = get_model_ppl(save_dir,egs_conf)
...
trainer = trainer.SimpleTrainer_ppl(package)
# trainer = trainer.SimpleTrainer_ppl2(package)

# loss_func_ce,loss_func_ppl = loss_func
# package = ({"data":data, "model":model,"loss_func":loss_func_ce,"loss_func_ppl":loss_func_ppl,"optimizer":optimizer,"lr_scheduler":lr_scheduler},
#         {"model_dir":save_dir, "epochs":epochs, "gpu_id":gpu_id,"compute_accuracy":True,"start_epoch":start_epoch,
#         "suffix":suffix,"accum_grad": accum_grad,"report_interval_iters":valid_iter,"use_tensorboard":True})
#
# trainer = trainer.SimpleTrainer_ppl(package)

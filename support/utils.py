...
def select_model_device(model, use_gpu, gpu_id="", benchmark=False):
    ...
    if use_gpu:
        ...
        if len(gpu_id) > 0 and use_ddp():
            ...
            model.cuda()
            model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[gpu_id[dist.get_rank()]],
...
def to_device(device_object, tensor):
    ...
    if isinstance(device_object, torch.nn.Module):
        # try:
        # print("device_object:all,parameters",device_object,next(device_object.parameters()))
        # exit()
        # device = device_object.parameters()
...
def for_device_free(function):
    ...
    Used in libs.model.framework.TopVirtualNnet
    """
...
def create_model_from_py(model_blueprint, model_creation=""):
    """ Used in pipeline/train.py and pipeline/onestep/extract_emdeddings.py and it makes config of model
    more free with no-change of training and other common scripts.

    @model_blueprint: string type, a *.py file path which includes the instance of model, such as examples/xvector.py
    @model_creation: string type, a command to create the model class according to the class declaration
                     in model_blueprint, such as using 'Xvector(40,2)' to create an Xvector model.
        ...
        if k in params_keys:
            if isinstance(v, type(params[k])):
                ...
                raise ValueError("The value type of default params [{0}] is "
                                 "not equal to [{1}] of params for k={2}".format(type(default_params[k]),
                                                                                 type(params[k]), k))
...
### Multi-GPU training [Two solutions: Horovod or DDP]
def init_multi_gpu_training_offline(gpu_id="", solution="ddp", port=29500):
    num_gpu = len(parse_gpu_id_option(gpu_id))
    if num_gpu > 1:
        # The DistributedDataParallel (DDP) solution is suggested.
        if solution == "ddp":
            init_ddp_offline(port)
            if is_main_training(): logger.info("DDP has been initialized.")
        elif solution == "horovod":
            init_horovod()
            if is_main_training(): logger.info("Horovod has been initialized.")
...
## DDP
def init_ddp_offline(port=29500):
    if not torch.distributed.is_nccl_available():
        raise RuntimeError("NCCL is not available.")
...
def csv_to_list(file):
    ...
    with open(file, newline="") as csvfile:
        ...
        for rows in reader:
            # {'eg-id,ark-path,start-position,end-position,class-label':
            # 'zho-nan,lre17_mfcc/data/dev/data/lre17_xrwmfqel,1800,2000,13'}
            # print("rows:",rows)
            lists.append(rows)
    return lists
...
def read_wav_list_test(wav_file,wav_root):
    wav_list = []
    with open(wav_file, 'r', encoding='utf8') as fin:
        for line in fin:
            line = line.strip()
            ...
            wav_path = line
            item['eg-id'] = line
            item['wav-path'] = os.path.join(wav_root, wav_path)
            item['class-label'] = 0
            wav_list.append(item)
    return wav_list

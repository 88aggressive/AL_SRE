...
class SpeechAug(torch.nn.Module):
    ...
    def __init__(self, aug_classes=[], mod="random"):
        ...
        # print("1aug_classes,", aug_classes)
        for aug_class in aug_classes:
            # print("1aug_class,",aug_class)
    ...
    def _random_forward(self, waveforms, lengths):
        ...
        if aug_idx == 0:
            # print("test:add no aug...")
            return waveforms, lengths
        else:
            # print("test:add random aug...")
...
class SpeechAug_noprint(torch.nn.Module):
    """This class implement three types of speech augment (chain,random,concat).
    ...
    def _random_forward(self, waveforms, lengths):
        ...
        if aug_idx == 0:
            ...
            if (torch.any((torch.isnan(waves)))):
                raise ValueError('random_1:{},type:{},typename:{}'.format(waveforms, self.augment[aug_idx - 1],
                                                                          self.augment_name[aug_idx - 1]))
            return waves, lengths
    ...
    def _concat_forward(self, waveforms, lengths):
        ...
        for count, augment in enumerate(self.augment):
            ...
            if (torch.any((torch.isnan(wavs_aug)))):
                raise ValueError(
                    'concat:{},type:{},typename:{}'.format(waveforms, self.augment[count], self.augment_name[count]))
            wavs_aug_tot.append(wavs_aug)
        waveforms = torch.cat(wavs_aug_tot, dim=0)
        lengths = torch.cat([lengths] * len(wavs_aug_tot))
        return waveforms, lengths
    ...
    def _chain_forward(self, waveforms, lengths):
        ...
        for count, augment in enumerate(self.augment):
            ...
            if (torch.any((torch.isnan(waveforms)))):
                raise ValueError(
                    'chian:{},type:{},typename:{}'.format(waveforms, self.augment[count], self.augment_name[count]))

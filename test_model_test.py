...
save_file = f"{save_dir}/scores_{epochs}_{trials_type}_spks{args.enroll_spks}_utts{enroll_nums}_{args.speech_aug}.txt"
#### Set seed
...
spk_file = f"{args.train_dir}/list/enroll_spks{args.enroll_spks}_utts{args.enroll_nums}.lst"
save_spk_path = f"{save_dir}/emds_spks_al_{epochs}_spks{args.enroll_spks}_utts{enroll_nums}.npy"

trials = f"{args.train_dir}/list/test_spks{args.enroll_spks}_utts{args.enroll_nums}.lst"
...
if not os.path.exists(save_file):
    with open(save_file,"w") as s_file:
        ...
        aug_reader = None
        # save_aug_audio = True  # True,False
        # print(speech_aug)
        if speech_aug == "true":
...
y_labels, y_scores = preparedata3(save_file)
...
save_eer = f"{save_dir}/eer_{epochs}_{trials_type}_spks{args.enroll_spks}_utts{args.enroll_nums}_{args.speech_aug}.txt"
